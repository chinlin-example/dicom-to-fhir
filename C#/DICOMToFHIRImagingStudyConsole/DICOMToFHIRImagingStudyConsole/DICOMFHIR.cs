...
namespace DICOMToFHIRImagingStudyConsole
{
    class DICOMFHIR
    {
        ...
        public string GetDICOMFHIRImagingStudyJson() 
        {
            ...
            List<Identifier> identifierList = FHIR.GetIdentifier(studyInstanceUID: studyInstanceUID, accessionNumberAndIssuer: accessionNumberAndIssuer, studyID: studyID);
            ...
            imagingStudy.Subject = FHIR.GetImagingStudySubject();
            imagingStudy.Started = FHIR.GetImagingStudyStarted();
            
            imagingStudy.NumberOfSeries = DICOM.GetNumberOfSeries();
            imagingStudy.NumberOfInstances = DICOM.GetNumberOfInstance();
            imagingStudy.Series = new List<ImagingStudy.SeriesComponent>() { FHIR.GetSeries() };
            imagingStudy.Series[0].Instance = new List<ImagingStudy.InstanceComponent>() { FHIR.GetInstance() };
